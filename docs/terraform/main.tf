# Telematics Insurance System - AWS Infrastructure
# Optimized for Free Tier and Cost Efficiency
...
resource "aws_db_instance" "postgres" {
  ...
  skip_final_snapshot    = true  # For dev/test
...
resource "aws_sqs_queue" "telematics_events" {
...
resource "aws_sqs_queue" "telematics_events_dlq" {
...
# IAM Policy for ECS tasks to access SQS, RDS, Redis
    ...
    Statement = [
      {
        ...
        Resource = aws_sqs_queue.telematics_events.arn
      },
      {
        Effect = "Allow"
        Action = [
  ...
  container_definitions = jsonencode([
    {
      ...
      environment = [
        ...
        {
          name  = "SQS_QUEUE_URL"
          value = aws_sqs_queue.telematics_events.url
        }
      ]
...
# Lambda Function for Event Processing (Serverless alternative)
...
resource "aws_apigatewayv2_api" "main" {
  ...
  protocol_type = "HTTP"
  description   = "Telematics Insurance API"
...
# EC2 Instance for Simulator (Free Tier: t3.micro)
resource "aws_security_group" "ec2" {
  name        = "${var.project_name}-ec2-sg"
  description = "Security group for EC2 simulator"
  vpc_id      = aws_vpc.main.id
...
resource "aws_instance" "simulator" {
  ...
  # key_name is optional - comment out if not using SSH
  ...
  user_data = <<-EOF
              ...
              # Docker Compose will be installed via script
              EOF
...
output "ec2_simulator_public_ip" {
  description = "EC2 simulator public IP"
  value       = aws_instance.simulator.public_ip
}

output "ecr_simulator_repository_url" {
  description = "ECR simulator repository URL"
